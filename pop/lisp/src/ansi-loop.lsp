#| --- Copyright MIT (1986) and Symbolics Inc. (1989-1992) -----------
 | File:            C.all/lisp/src/ansi-loop.lsp
 | Purpose:         Loop macro
 | Author:          See below (installed by John Williams, May  5 1995)
...
#+Poplog
(make-package :ansi-loop)

#+Poplog
(progn
    (proclaim '(ftype (function (string string &rest t) t)  string=))
    (proclaim '(ftype (function (t) t)                      type-of))
    (proclaim '(ftype (function (t sequence &rest t) t)     find delete))
    )

#+Poplog
(defmacro ECASE (keyform &rest clauses)
    ;; Early version of ECASE
    `(let ((#1=#:key ,keyform))
        (case #1#
            ,@clauses
            (t (error "Got ~S, expecting one of ~S"
                      #1# ',(mapcar #'car clauses))))))
...
(defun loop-optimization-quantities (env)
  ...
  (declare #-(or ANSI Poplog) (ignore env)
...
(defun hide-variable-references (variable-list form)
  (declare #-(or Genera Poplog) (ignore variable-list))
...
(defun hide-variable-reference (really-hide variable form)
  (declare #-(or Genera Poplog) (ignore really-hide variable))
...
(defun make-standard-loop-universe (&key keywords for-keywords iteration-keywords path-keywords
  ...
  #-(or Poplog (and CLOE Source-Bootstrap)) (check-type ansi (member nil t :extended))
...
#+Poplog
(proclaim '(ftype (function (t t) t) estimate-code-size estimate-code-size-1))
...
#+Poplog (proclaim '(ftype (function (t t) t) loop-declare-variable))
...
(defun loop-for-on (var val data-type)
  (multiple-value-bind (list constantp list-value) (loop-constant-fold-if-possible val)
    ...
    #-Poplog (declare #+(and (not LOOP-Prefer-POP) (not CLOE)) (ignore step-function))
...
(defun loop-for-in (var val data-type)
  (multiple-value-bind (list constantp list-value) (loop-constant-fold-if-possible val)
    (let ((listvar (loop-gentemp 'loop-list-)))
      ...
      (multiple-value-bind (list-step step-function) (loop-list-step listvar)
    #-(or LOOP-Prefer-POP Poplog) (declare (ignore step-function))
...
(defun add-loop-path (names function universe &key preposition-groups inclusive-permitted user-data)
  ...
  #-(or Genera Poplog (and CLOE Source-Bootstrap)) (check-type universe loop-universe)
...
(defun loop-hash-table-iteration-path (variable data-type prep-phrases &key which)
  #-Poplog (check-type which (member hash-key hash-value))
...
;;;INTERFACE: ANSI
#+Poplog (fmakunbound 'loop)
#+Poplog (fmakunbound 'ecase)
