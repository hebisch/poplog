...
static void
ClearRectangle(Display * dpy, Window win, MyGC mygc, XRectangle * xcliprect,
              Coord x, Coord y, int width, int height)
{
    GC gc = mygc.gc;
...
#define XDRAWSTRING(x, y, str, n)                                       \
    if (fontmode & FDM_1BY1) {                                          \
        DrawString1By1(dpy, win, gc, x, y, str,n, font, fwidth);        \
    } else {                                                            \
        XDrawString(dpy, win, gc, x, y, (char*)str, n);                 \
    }
...
*/
static void
DrawString1By1(Display * dpy, Window win, GC gc, int x, int y,
               uchar * string, int nchars, XFontStruct * font, int fwidth)
{
...
static void
wcDrawString1By1(Display * dpy, Window win, GC gc, int x, int y,
                 wchar_t * string, int nchars, XFontSet fontset, int fwidth)
{
...
static void
ucDrawString1By1(Display * dpy, Window win, GC gc, int x, int y,
                 ushort *string, int nchars, XFontSet fontset, int fwidth)
{
...
static int
XucTextEscapement(XFontSet fontset, ushort * string, uint nchars)
{
    uchar mbout[MAX_COLS*4];
    uint nbytes = (*XpwUcToMb)(string, nchars, mbout, MAX_COLS*4);
    return(XmbTextEscapement(fontset, (char *)mbout, nbytes));
}


#define UC_DRAWSTRING(Name, Draw)                               \
static void Name(Display * dpy, Window win, XFontSet fontset, GC gc,  \
                 int x, int y, ushort * string, uint nchars)    \
{                                                               \
    uchar mbout[MAX_COLS*4];                                    \
    uint nbytes = (*XpwUcToMb)(string, nchars, mbout, MAX_COLS*4);  \
    Draw(dpy, win, fontset, gc, x, y, (char *)mbout, nbytes);       \
}
 ...
 */
static void
DrawCursor(XpwScrollTextWidget w, GC gc, int xleft, int ybase,
           uchar cchar, int cwidth, Boolean bold)
{
    Display *dpy = XtDisplay(w);
    Window win = XtWindow(w);
    int fheight = FontHeight(w),
        fbaseline = FontBaseline(w), ytop = ybase - fbaseline,
        xmid = xleft+(cwidth-1)/2,
...
*/
static int
DrawGraphString(XpwScrollTextWidget w, GC gc, int xleft, int ybase,
                uchar * string, uint nchars, Boolean bold,
                XFontStruct * font, int fontmode, Boolean var_width_mode)
{
    Display *dpy = XtDisplay(w);
    Window win = XtWindow(w);
...
static Coord
DrawString(XpwScrollTextWidget w, uchar * string, Coord x, uint col, uint row,
           uint nchars, uint attrs, uint flags, uint cursorchar)
{
    Display *dpy = XtDisplay(w);
    Window win = XtWindow(w);
    int
    ...
    if (attrs & ATTR_ACTIVE) {
        ...
        if (!(flags & DS_ISCURSOR)) {
            gc_index |= ATTR_ACTIVE;
        }
    }
    ...
    if (flags & DS_NO_DRAW) {
        ...
        */
        ...
        if (actmode & ACT_INVERT) {
            invert = !invert;
        }
        if (invert) {
            mygc.flags &= ~(GC_HAS_WIN_BG|GC_HAS_BG_PIX);
        } else if (mygc.flags & GC_HAS_BG_PIX) {
            fontmode |= FDM_CLEAR_BG;
        }
    ...
    while (p < plim) {                                                      \
        ...
        if (n = p - pstart) {                                               \
            ...
            if (cursorchar) {                                               \
                ...
                if (draw) {                                                 \
                    DRAWSTRING(x, ybase, pstart, n);                        \
                }                                                           \
                if (overstrike) {                                           \
                    DRAWSTRING(x+1, ybase, pstart, n);                      \
                }                                                           \
            }                                                               \
            x += twidth;                                                    \
...
#define SVGCFONT_DRAWLOOP(TEXTWIDTH, DRAWSTRING, DRAWIMAGESTRING)           \
    ...
    if (fontmode <= FDM_NO_DRAW) {
        if (fontmode == FDM_NO_DRAW1 && x > draw_lim_x) {
            draw_lim_col = col-1;
        }
    ...
    if (actmode & ACT_BOX) {
        ...
        if (startcol == onactive_start_col) {
            SETLSEG(start_x, y, start_x, ybot);
        }
        if (col == onactive_lim_col) {
            SETLSEG(xright,  y, xright,  ybot);
        }
        XDrawSegments(dpy, win, gc, lseg, lsegp-lseg);
    }
...
static Coord
DrawSpaces(XpwScrollTextWidget w, Coord x, uint col, uint row, uint nchars)
{
    Display *dpy = XtDisplay(w);
    Window win = XtWindow(w);
    int start_x = x;
...
static Coord
DrawRowSequences(XpwScrollTextWidget w, int x, uint scol, uint lcol,
                 uint row, uint attr_mask, uint ds_flags)
{
    ...
    if (VarWidthMode(w)) {
        if (scol < fixed_cols && fixed_cols < lcol) {
            x = DrawRowSequences(w, x, scol, fixed_cols, row, attr_mask,
                                                                ds_flags);
            if (ds_flags&DS_X_LIM && draw_lim_col >= 0) {
                return(x);
            }
            scol = fixed_cols;
        }
        if (scol >= fixed_cols) {
            sp_width = SpaceWidth(w);
        }
    }
...
#define DRAW_ROW_SEQS(RPCHAR)                                               \
    while (rp <= rplim) {                                                   \
        if (rp >= rpclim || (wc = RPCHAR) == 0 || rp->attrs != attrs) {     \
            if (len != 0) {                                                 \
                uint dsf = ds_flags;                                        \
                if (attr_mask!=0 && !(attrs&attr_mask)) {                   \
                    dsf |= DS_NO_DRAW;                                      \
                }                                                           \
...
static Coord
ColToXCoord(XpwScrollTextWidget w, uint col, uint row) {
    if (VarWidthMode(w)) {
...
static Coord
GetCursorX(XpwScrollTextWidget w) {
    Coord x;
...
int
_XpwXCoordToCol(XpwScrollTextWidget w, Coord x, uint row) {
...
static int
AreaCoordToRow(XpwScrollTextWidget w, Coord y, Boolean in_status) {
...
int
_XpwYCoordToRow(XpwScrollTextWidget w, Coord y) {
    int row;
 ...
 */
void
_XpwTextExpose(XpwScrollTextWidget w, Coord x, Coord y, Coord xl, Coord yl,
              Boolean in_status)
{
    int fwidth = FontWidth(w), fheight = FontHeight(w),
    ...
    while (srow < lrow) {
        ...
        if (scol < lcol) {
            ...
            if (srow == cursor_row && scol <= cursor_col
                  && cursor_col < lcol) {
...
static void
DrawRowChars(XpwScrollTextWidget w, int x, uint scol, uint lcol, uint row)
{
...
static void
ClearOnActive(XpwScrollTextWidget w) {
...
void
_XpwTextTestActiveChange(XpwScrollTextWidget w) {
    ...
    if (set_new) {
        ...
        if (csz_code == CSZ_BYTE) {
            ...
            while (lcol < nchars && (rp[lcol].attrs & ATTR_ACTIVE)) {
                lcol++;
            }
        } else if (csz_code == CSZ_SHORT) {
            do col--; while (0 <= col && (rpS[col].attrs & ATTR_ACTIVE));
            while (lcol < nchars && (rpS[lcol].attrs & ATTR_ACTIVE)) {
                lcol++;
            }
        } else {
...
static void
EnsureBlinkWidget(XpwScrollTextWidget w) {
...
static int write_string(XpwScrollTextWidget w, Coord x, uint col, uint row,
         int offset, int len, uchar * string, Boolean at_cursor, int mode);
...
static void Name(WinChar_t * sp, WinChar_t * dp, int len) {  \
...
static void Name(WinChar_t * dp, int len) {     \
    register WinChar_t *dlimp = dp+len;         \
    while (dp < dlimp) {                        \
...
static void
winrow_copy(WinRowB * sp, WinRowB * dp, int len) {
    WinRowB *slimp = sp+len;
    if (dp < sp) {
        while (sp < slimp) *dp++ = *sp++;
    } else {
        WinRowB *dlimp = dp+len;
        while (slimp > sp) {
            *--dlimp = *--slimp;
        }
    }
...
static WinRowB
expand_row(XpwScrollTextWidget w, uint row, uint want_cols) {
    ...
    if (!free) {
        ...
        if (csz_code == CSZ_BYTE) {
            ...
            wincharI_copy((WinRowI)rptr, (WinRowI)new_rptr, nchars);
        }
    }
    return(new_rptr);
}
...
static void
clear_chars(XpwScrollTextWidget w, int scol, int srow, int ncols, int nrows,
            Boolean set_end)
{
    ...
    while (nrows--) {
        ...
        if (csz_code == CSZ_BYTE) {
            ...
            if (ncols == 1) {
                ...
                wincharB_zero(rpB, ncols);
            }
        } else if (csz_code == CSZ_SHORT) {
            WinRowS rpS = (WinRowS)rptr + scol;
            if (ncols == 1) {
                rpS->c0 = rpS->c1 = rpS->attrs = 0;
            } else {
                wincharS_zero(rpS, ncols);
            }
        } else {
            WinRowI rpI = (WinRowI)rptr + scol;
            if (ncols == 1) {
                rpI->s0 = rpI->s1 = rpI->attrs = 0;
            } else {
                wincharI_zero(rpI, ncols);
            }
        }
    }
}
...
static void
clear_text(XpwScrollTextWidget w, int scol, int srow, int ncols, int nrows)
{
    Boolean whole_rows;
...
static void
copy_row_chars(XpwScrollTextWidget w, int scol, int srow,
               int dcol, int drow, int ncols)
{
    WinRowB *rowarr = RowArray(w);
    WinRowB srp = rowarr[srow], drp =  rowarr[drow];
    uint csz_code = CharSizeCode(w);
    int dlcol, trail0, nchars;
    ...
    if (csz_code == CSZ_BYTE) {
        ...
        if (trail0 != 0) {
            wincharB_zero(drp+ncols, trail0);
        }
    } else if (csz_code == CSZ_SHORT) {
        WinRowS srpS = (WinRowS)srp + scol,
                drpS = (WinRowS)drp + dcol;
        wincharS_copy(srpS, drpS, ncols);
        if (trail0 != 0) {
            wincharS_zero(drpS+ncols, trail0);
        }
    } else {
        WinRowI srpI = (WinRowI)srp + scol,
                drpI = (WinRowI)drp + dcol;
        wincharI_copy(srpI, drpI, ncols);
        if (trail0 != 0) {
            wincharI_zero(drpI+ncols, trail0);
        }
    }
}

static void
copy_text(XpwScrollTextWidget w, int scol, int srow, int ncols, int nrows,
          int c_dis, int r_dis)
{
    int dcol = scol+c_dis, lrow = srow+nrows, drow = srow+r_dis;
    ...
    if (!VarWidthMode(w) && ncols == NumCols(w)) {
        ncols = MAX_COLS;
    }
...
static Coord
draw_text(XpwScrollTextWidget w, uchar * string,
          int dx, int dcol, int drow, int len)
{
    int dlcol = dcol+len, attrs = w->xpwscrolltext.curr_attributes, nchars;
    uint csz_code = CharSizeCode(w);
    WinRowB rptr = RowArray(w)[drow];
...
static uint
winchar_at(XpwScrollTextWidget w, int col, int row, uchar * attr_p)
{
...
static void
Clear(XpwScrollTextWidget w, int col, int row, int ncols, int  nrows) {
...
static void
ClearScreen(XpwScrollTextWidget w) {
    Clear(w, 0, 0, 0, 0);
}

static void
ClearLine(XpwScrollTextWidget w, int start_line) {
    Clear(w, 0, start_line, 0, 1);
}


static void
ClearLines(XpwScrollTextWidget w, int start_line, int num_lines) {
    Clear(w, 0, start_line, 0, num_lines);
}

static void
ClearTail(XpwScrollTextWidget w, int col, int row) {
    Clear(w, col, row, 0, 1);
}

static void
ClearTails(XpwScrollTextWidget w, int col, int row, int num_lines) {
    Clear(w, col, row, 0, num_lines);
}

static void
ClearChar(XpwScrollTextWidget w, int col, int row) {
    Clear(w, col, row, 1, 1);
}

static void
ClearLineAtCursor(XpwScrollTextWidget w) {
    Clear(w, 0, CursorRow(w), 0, 1);
}

static void
ClearTailAtCursor(XpwScrollTextWidget w) {
    Clear(w, CursorCol(w), CursorRow(w), 0, 1);
}

static void
ClearCharAtCursor(XpwScrollTextWidget w) {
...
static void
VarTextScroll(XpwScrollTextWidget w, int row, int nrows,
              int c_dis, Boolean save_cursor)
{
...
static void
Scroll(XpwScrollTextWidget w, int col, int row, int ncols,
                   int nrows, int c_dis, int r_dis)
{
    ...
    */
    ...
    if (row+nrows+r_dis >= num_rows) {
        nrows = num_rows - (row+r_dis);
    }
    ...
    if (ncols == 0) {
        ...
        ncols = min(ncols, num_cols - col);
    }
    if (col+ncols+c_dis >= num_cols) {
        ncols = num_cols - (col+c_dis);
    }
...
static void ScrollScreenUp(XpwScrollTextWidget w) {
    Scroll(w, 0, 1, 0, 0, 0, -1);
}

static void
ScrollScreenDown(XpwScrollTextWidget w) {
    Scroll(w, 0, 0, 0, 0, 0, 1);
}

static void
ScrollScreenLeft(XpwScrollTextWidget w) {
    Scroll(w, 1, 0, 0, 0, -1, 0);
}

static void
ScrollScreenRight(XpwScrollTextWidget w) {
    Scroll(w, 0, 0, 0, 0, 1, 0);
}

static void
ScrollLines(XpwScrollTextWidget w, int start_line, int num_lines, int dist) {
    Scroll(w, 0, start_line, 0, num_lines, 0, dist);
}

static void
ScrollTail(XpwScrollTextWidget w, int col, int row, int dist) {
    Scroll(w, col, row, 0, 1, dist, 0);
}

static void
ScrollTails(XpwScrollTextWidget w, int col, int row, int num_rows,
           int dist)
{
...
static void
ScrollTailLeft(XpwScrollTextWidget w, int col, int row) {
    Scroll(w, col, row, 0, 1, -1, 0);
}

static void
ScrollTailRight(XpwScrollTextWidget w, int col, int row) {
    Scroll(w, col, row, 0, 1, 1, 0);
}

static void
InsertLineAtCursor(XpwScrollTextWidget w) {
    Scroll(w, 0, CursorRow(w), 0, 0, 0, 1);
}

static void
DeleteLineAtCursor(XpwScrollTextWidget w) {
    int row = CursorRow(w);
    if (row == NumRows(w)-1) {
        Clear(w, 0, row, 0, 1);
    } else {
...
static uchar *
increment_string(XpwScrollTextWidget w, uchar * string, int offset) {
...
static int
process_string(XpwScrollTextWidget w, int offset, int len, uchar ** stringp)
{
...
static int
write_string(XpwScrollTextWidget w, Coord x, uint col, uint row, int offset,
             int len, uchar * string, Boolean at_cursor, int mode)
{
    Display *dpy = XtDisplay(w);
    Window win = XtWindow(w);
    ...
    if (string) {
        len = process_string(w, offset, len, &string);
    }
    ...
    if (!VarWidthMode(w)) {
        ...
        if (flen > 0) {
            ...
            len -= flen;
            if (len > 0 && string) {
                string = increment_string(w, string, flen);
            }
        }
        if (len > 0) {
            Coord endx;
...
/*
                        if (limx != Limx) {
                            _XpwTextExpose(w, x, y, xl, y+FontHeight(w),
                                           row==0);
                        }
*/
                    } else if (oldend < endx) {
    ...
    if (at_cursor) {
        CursorCol(w) = col+len; CursorX(w) = x;
    }
    if (copywait) {
        _XpwTextCopyWait(w);
    }
...
static XpwMethodRet
Insert(XpwScrollTextWidget w, int col, int row, uchar * string, int len) {
    Boolean DoCursor, save_cursor;

    if (row >= NumRows(w) || row < 0 || col < 0) {
        return(0);
    }
...
static void
InsertAtCursor(XpwScrollTextWidget w, uchar * string, int len) {
    Boolean save_cursor = _XpwTextCursorOn(w, False);
...
static void
InsertCharAtCursor(XpwScrollTextWidget w) {
    Boolean save_cursor = _XpwTextCursorOn(w, False);
...
static void
Write(XpwScrollTextWidget w, int col, int row, int start_line,
      int nlines, uchar * strings[], int offset, int len, int clear)
{
    Boolean DoCursor = False, save_cursor;
    int i, cursor_row = CursorRow(w), num_rows = NumRows(w);

    if (row >= num_rows || row < 0 || col < 0) {
        return;
    }
    if (row + nlines > num_rows) {
        nlines = num_rows - row;
    }
...
static void
WriteLine(XpwScrollTextWidget w, int col, int row, uchar * string,
          int len, int clear)
{
    uchar *strings[1];
    strings[0] = string;
...
static void
WriteLines(XpwScrollTextWidget w, int col, int row, int start_line,
           int num_lines, uchar * strings[], int len, int clear)
{
...
static void
WriteSubstr(XpwScrollTextWidget w, int col, int row, uchar * string,
            int offset, int len, int clear)
{
    uchar *strings[1];
    strings[0] = string;
...
WriteAtCursor(XpwScrollTextWidget w, uchar * string, int len)
{
    ...
    if (no_draw_mode & 1) {
        return(0);    /* already passed limit */
    }

    if (string) {
        len = process_string(w, 0, len, &string);
    }
...
static XpwMethodRet
BeginTextWidthMode1(XpwScrollTextWidget w) {
...
static XpwMethodRet
SetVarColumnOffset(XpwScrollTextWidget w, int row, int ncols, int pixrem)
{
    WinRowB rptr = RowArray(w)[row];
    ...
    if (PIXOFFS(rptr) > 0 && diff > 0 && nchars > 0) {
        ...
        */
        ...
        if (csz_code == CSZ_BYTE) {
            WinRowB rpB = rptr + num_fixed;
            while (len--) {
                rpB->c0 = 0; rpB->attrs = 1; rpB++;
            }
        } else if (csz_code == CSZ_SHORT) {
            WinRowS rpS = (WinRowS)rptr + num_fixed;
            while (len--) {
                rpS->c0 = rpS->c1 = 0; rpS->attrs = 1; rpS++;
            }
        } else {
            WinRowI rpI = (WinRowI)rptr + num_fixed;
            while (len--) {
...
static XpwMethodRet
GetVarColumnOffset(XpwScrollTextWidget w, int row, int * pixrem_p) {
    WinRowB rptr = RowArray(w)[row];
...
static XpwMethodRet
GetVarColumnLimOffset(XpwScrollTextWidget w, int row, int * pixrem_p) {
    WinRowB rptr = RowArray(w)[row];
...
static XpwMethodRet
GetVarRowWidth(XpwScrollTextWidget w, int row) {
...
static void
DeleteCharsAtCursor(XpwScrollTextWidget w, int nchars) {
    if (VarWidthMode(w)) {
        Boolean save_cursor = _XpwTextCursorOn(w, False);
...
static void
DeleteCharAtCursor(XpwScrollTextWidget w) {
...
Boolean
_XpwTextCursorOn(XpwScrollTextWidget w, Boolean on) {
    ...
    if (w->xpwscrolltext.no_draw_mode >= 0) {
        return(wason);
    }
...
XpwMethodRet
_XpwTextMoveCursorTo(XpwScrollTextWidget w, int col, int row) {
...
static void
MoveCursorUp(XpwScrollTextWidget w) {
    _XpwTextMoveCursorTo(w, CursorCol(w), CursorRow(w)-1);
}

static void
MoveCursorDown(XpwScrollTextWidget w) {
    _XpwTextMoveCursorTo(w, CursorCol(w), CursorRow(w)+1);
}

static void MoveCursorLeft(XpwScrollTextWidget w) {
    _XpwTextMoveCursorTo(w, CursorCol(w)-1, CursorRow(w));
}

static void MoveCursorRight(XpwScrollTextWidget w) {
    _XpwTextMoveCursorTo(w, CursorCol(w)+1, CursorRow(w));
}


static void
Bell(Widget w, int volume) {
...
static setsel_context *
GetSelnContext(Widget w, XpwAssocID selection, Boolean create) {
    setsel_context *c, *context;

    /* build table first time */
    if (!setsel_table) {
        setsel_table = XpwCreateAssocTable(4);
    }
...
static Boolean
SelectionConvert(Widget w, XpwAssocID * selectionp, Atom * target,
                 Atom * type_return, Opaque * value_return,
...
/* Losing selections */
static void
LoseSelection(Widget w, XpwAssocID * selectionp)
{
    setsel_context *context = GetSelnContext(w, *selectionp, False);
...
XpwMethodRet
XpwSetSelection(Widget w, String location, String string, int nbytes,
                XtCallbackProc lose_p, Opaque client)
{
    Display *dpy = XtDisplay(w);
    Atom selection;
...
XpwMethodRet
XpwGetSelection(Widget w, String location, XtCallbackProc receive_p,
                Opaque client)
{
    Display *dpy = XtDisplay(w);
