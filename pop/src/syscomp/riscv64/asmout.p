...
define asm_startfile(name);
    ...
    asmf_printf('\t.option nopic\n');
    asmf_printf('\t.attribute arch, "rv64imafdc_zicsr_zifencei"\n');
enddefine;
...
define asm_gen_poplink_code(outlabs, nfroz, jmplab);
    lvars outlabs, nfroz, jmplab, l, l0, offs;
    ;;; plant exec labels
    outlabs();
    ;;; Push nfroz frozvals (last nfroz longwords planted)
    ;;; 's0' is the user stack pointer
    asmf_pr('\tauipc   a2, 0\n');
    asmf_pr('\tmv a1, a2\n');
    fast_for offs from nfroz*8 by -8 to 8 do
        asmf_pr('\taddi a1, a1, -8\n');
        asmf_pr('\tld a0, (a1)\n');
        asmf_pr('\taddi s0, s0, -8\n');
        asmf_pr('\tsd a0, (s0)\n');
    endfast_for;
    ;;; Jump to -jmplab-
    ;;; nextlab() -> l;
    ;;; asmf_printf(l, l0, '\tld a0, (%p - %p)(a2)\n');
    asmf_printf('\tauipc   a0, 0\n');
    asmf_printf('\tld a0, 12(a0)\n');
    asmf_pr('\tjr a0\n');
    ;;; asm_outlab(l);
    asm_align_word();
 ...
 *  padded to exactly 24 bytes.  It must preserve argument registers
 *  and call-preserved registers.  On Risc-V we have several possible
 *  choices for work register, we use t1.
 */

define asm_gen_exfunc_clos_code(action_lab);
    lvars action_lab, l = nextlab();
    ;;; the following would not work, as we copy the template
    ;;; version using 'lui' would work in first 2 Gb
    ;;; asmf_printf(action_lab, '\tauipc t1, %%hi(%p)\n');
    ;;; asmf_printf('\t.align\t2\n');
    ;;; asmf_printf(action_lab, '\tjalr t1, %%lo(%p)t1\n');
    ;;; asmf_printf('\t.align\t2\n');
    asmf_printf('\tauipc t1, 0\n');
    asmf_printf('\tld t1, 16(t1)\n');
    asmf_printf('\tjalr t1, t1\n');
    asmf_printf('\t.align\t3\n');
    asm_outlab(l);
