...
define asm_gen_poplink_code(outlabs, nfroz, jmplab);
    ...
    ;;; 'x20' is the user stack pointer
    asm_outlab(nextlab() ->> l);
    fast_for offs from nfroz*8 by -8 to 8 do
        asmf_printf(asm_expr(l, "-", offs), '\tldr x0, %p\n');
        asmf_pr('\tstr x0, [x20, #-8]!\n');
    endfast_for;
    ;;; Jump to -jmplab-
    nextlab() -> l;
    asmf_printf(l, '\tldr x0, %p\n');
    asmf_pr('\tbr x0\n');
    asm_outlab(l);
 ...
 *  and call-preserved registers.  On ARM64 we have several possible
 *  choices for work registers, we use x16 and x17 as that is also used
 *  by standard PLT code.
 */

define asm_gen_exfunc_clos_code(action_lab);
    lvars action_lab, l = nextlab();
    asm_outlab(l);
    asmf_printf(l, '\tadr x17, %p\n');
    nextlab() -> l;
    asmf_printf(l, '\tldr x16, %p\n');
    asmf_printf('\tbr x16\n');
    asm_outlab(l);
