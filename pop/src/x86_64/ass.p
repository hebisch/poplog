...
define constant is_large_int(_src);
  lvars src;
    if _src _slt _-2147483648 or _src _sgr _2147483647 then
      true
    else
      false
    endif
...
define constant Get_regindir_efa(_reg, _disp);
    lvars _reg, _disp;
    if _zero(_disp) and _reg /== _EBP and _reg /== _R13 then
        _DISP0
...
define constant Drop_pfx(opcodes, _rm);
    lvars opcodes, _rex = Get_rex_short(_rm), _code;
    until (fast_destpair(opcodes) ->> opcodes) == [] do
        _int(/* prefix */) -> _code;
        if _code == _MOVX then
            Drop_b(_rex);
            _0 -> _rex;
        elseif _code == _WORD then
            ;;; Clear REX_W bit
...
define constant Drop_exptr_deref(exptr, _reg, _tmp_reg);
    lvars exptr, _reg, _tmp_reg;
    fast_repeat exptr times
        Drop_std(_MOV_IN, _tmp_reg, Get_regindir_efa(_reg, _0));
        _tmp_reg -> _reg;
    endrepeat
enddefine;

define constant Do_bitfield(type, _size, structure, offset, upd, exptr);
    lvars type, structure, offset, upd, exptr, _size, _disp, _reg;
    ...
    elseif Is_register(Trans_structure(structure)) ->> _reg then
        Drop_std(_MOV_IN, _EDI, _REG, _int(_reg), _0);
    else
        mishap(0, 'SYSTEM ERROR 1 IN Do_bitfield');
    endif;
    ;;; If it's an external structure, get the real address
    if exptr then
        Drop_exptr_deref(exptr, _EDI, _EDI)   ;;; deref exptr times
    endif;
    ...
    if isinteger(offset) then
        ...
        elseif Is_register(Trans_structure(offset)) ->> _reg then
            Drop_std(_MOV_IN, _EAX, _REG, _int(_reg), _0);
        else
...
define constant Do_field(type, structure, _size, offset, defer, upd, exptr);
    lvars   ops, type, structure, offset, defer, upd, exptr,
            _size, _reg, _disp, _n, _tmp_reg, _mod, _rm, _opcode ;
...
;;;        _extern fflush(_0) -> _;
    ...
    if upd then _EDI else _ESI endif ->> _reg -> _tmp_reg;
    if not(structure) then
        ;;; structure is on top of stack (from -sysFIELD_VAL-)
    ...
    elseif Is_register(Trans_structure(structure)) ->> _reg then
        _int(_reg) -> _reg;
        ;;; printf(upd, 'register in Do_field, upd = %p, ');
        ;;; printf(exptr, 'exptr= %p, ');
        ;;; printf(defer, 'defer = %p, ');
        ;;; printf(offset, 'offset= %p\n');
    else
        mishap(structure, 1, 'SYSTEM ERROR 1 IN Do_field');
    endif;
    ;;; If it's an external structure, get the real address
    if exptr then
        Drop_exptr_deref(exptr, _reg, _tmp_reg);   ;;; deref exptr times
        _tmp_reg -> _reg;
    endif;
    ...
    if isinteger(offset) then
        ...
        Get_regindir_efa(_reg, _disp) -> (_mod, _rm, _disp);
        Drop_pfx(ops, _rm) -> (_opcode, _rm);
        Drop_std(_opcode, _EAX, _mod, _rm, _disp);
    else
        ;;; Vector-type structure with index in source specified by offset
        ;;; Load index to ECX
        _ECX -> _tmp_reg;
        ...
        elseif Is_register(Trans_structure(offset)) ->> _tmp_reg then
            Drop_std(_MOV_IN, _ECX, _REG, _int(_tmp_reg), _0);
            ;;; mishap(offset, 1, 'Register offset in Do_field');
        else
...
;;;                _extern fflush(_0) -> _;
        ...
        Get_regindir_efa(_reg, _disp) -> (_mod, _rm, _disp);
        Drop_pfx(ops, _rm) -> (_opcode, _rm);
        Drop_extd(_opcode, _EAX, _mod, _rm, _disp, _ECX,
                  _int(index_scale(_n)));
    endif;
enddefine;
